...
from projects.mmdet3d_plugin.models.utils.misc import MLN, MLN_0, topk_gather, transform_reference_points, memory_refresh, SELayer_Linear
...
class Decoder(nn.Module):
    def __init__(self, dim=256, blocks=[128, 64, 32], out_dim=4 ,residual=True, factor=2):
        super().__init__()
...
class StreamPETRHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if self.is_seg: 
            self.seg_decoder = Decoder(dim=256, blocks=[128, 64, 32], out_dim=4)
    ...
    def _init_layers(self):
        ...
        # self.spatial_alignment = MLN_0(8)
    ...
    def position_embeding(self, data, memory_centers, topk_indexes, img_metas):
        ...
        img2lidars = torch.tensor(img2lidars, device='cuda')
        img2lidars = img2lidars.view(BN, 1, 1, 4, 4).repeat(1, H*W, D, 1, 1).view(B, LEN, D, 4, 4)
        img2lidars = topk_gather(img2lidars, topk_indexes)
    ...
    def forward(self, memory_center, img_metas, topk_indexes=None,  **data):
        ...
        # MODIFIED
        # del(data['extrinsics'])
        # data['intrinsics'] = torch.zeros_like(data['intrinsics']).to(data['img_feats'].device)
        # data['ego_pose'] = torch.eye(4).unsqueeze(0).to(data['img_feats'].device)
        # data['ego_pose_inv'] = torch.eye(4).unsqueeze(0).to(data['img_feats'].device)
        ...
        x = data['img_feats']
        B, N, C, H, W = x.shape
        # print('sample 0 CAM_FRONT_R timestamp: ', float(data['img_timestamp'][0][1].detach().cpu().numpy()))
        num_tokens = N * H * W
        ...
        # print(memory.shape, tgt.shape, query_pos.shape, pos_embed.shape, attn_mask.shape, temp_memory.shape, temp_pos.shape)
        ...
        if self.is_seg:
            ...
            for lvl in range(outs_dec.shape[0]):
                ...
                lane_queries_lvl = lane_queries[lvl].view(lane_queries[lvl].shape[0], 30, 30, -1).permute(0, 3, 1, 2)
                seg_pred = self.seg_decoder(lane_queries_lvl)
    ...
    def loss_single(self,
                    ...
                    gt_bboxes_ignore_list=None,
                    all_seg_preds=None,
                    all_gt_seg_list=None
                    ):
        """"Loss function for outputs from a singloss_singlele decoder layer of a single
        feature level.
        Args:
        ...
        # MAP MODIFIED
        # seg loss
        loss_seg = self.loss_seg(all_seg_preds, all_gt_seg_list)
    ...
    def loss(self,
        ...
        gt_seg = gt_seg.type(torch.LongTensor).to(device).squeeze(1)
        all_gt_seg_list = [gt_seg for _ in range(num_dec_layers)]

        losses_cls, losses_bbox, losses_seg = multi_apply(
            self.loss_single, all_cls_scores, all_bbox_preds,
            all_gt_bboxes_list, all_gt_labels_list, 
            all_gt_bboxes_ignore_list, all_seg_preds, all_gt_seg_list)
        ...
        num_dec_layer = 0
        for loss_cls_i, loss_bbox_i, loss_seg_i in zip(losses_cls[:-1],
                                           losses_bbox[:-1], losses_seg[:-1]):
