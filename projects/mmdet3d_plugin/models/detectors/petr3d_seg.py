...
class Petr3D(MVXTwoStageDetector):
    ...
    def forward_pts_train(self,
        ...
        if return_losses:
            loss_inputs = [gt_bboxes_3d, gt_labels_3d, outs, gt_seg]
    ...
    def forward_train(self,
        ...
        # MAP MODIFIED 添加 seg gt
        gt_seg = data['semantic_indices']
        ...
        losses = self.obtain_history_memory(gt_bboxes_3d,
                        ...
                        gt_labels, img_metas, centers2d, depths, gt_bboxes_ignore, gt_seg=gt_seg, **data)
    ...
    # MODIFIED
    def forward_test_my(self, img_metas, rescale, **data):
        self.test_flag = True
        
        # for key in list(img_metas[0][0].keys()):
        #     if key not in ['pad_shape', 'scene_token', 'box_type_3d']:
        #         img_metas[0][0].pop(key)
    ...
    def simple_test_pts(self, img_metas, **data):
        ...
        # MODIFIED
        # del(data['extrinsics'])
        # data['lidar2img'] = torch.zeros_like(data['lidar2img']).to(data['img_feats'].device)
        # data['lidar2img'][:, :] = torch.diag(torch.tensor([1.0, 1.0, 1.0, 1.0])).to(data['img_feats'].device)
        
        # data['lidar2img'] = torch.randn(data['lidar2img'].shape).to(data['img_feats'].device)
