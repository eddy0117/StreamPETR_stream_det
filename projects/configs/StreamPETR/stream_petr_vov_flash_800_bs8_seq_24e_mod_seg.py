...
queue_length = 1
num_frame_losses = 1
collect_keys=['lidar2img', 'intrinsics', 'extrinsics','timestamp', 'img_timestamp', 'ego_pose', 'ego_pose_inv', 'semantic_indices']
input_modality = dict(
    use_lidar=False,
    use_camera=True,
    use_radar=False,
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'nuscenes2d_map_temporal_infos_train.pkl',
        num_frame_losses=num_frame_losses,
        ...
        # MAP MODIFEID
        grid_conf=grid_conf),
    val=dict(type=dataset_type, grid_conf=grid_conf, pipeline=test_pipeline, collect_keys=collect_keys + ['img', 'img_metas'], queue_length=queue_length, ann_file=data_root + 'nuscenes2d_map_temporal_infos_val.pkl', classes=class_names, modality=input_modality),
    test=dict(type=dataset_type, grid_conf=grid_conf, pipeline=test_pipeline, collect_keys=collect_keys + ['img', 'img_metas'], queue_length=queue_length, ann_file=data_root + 'nuscenes2d_map_temporal_infos_val.pkl', classes=class_names, modality=input_modality),
...
# load_from='ckpts/fcos3d_vovnet_imgbackbone-remapped.pth'
# resume_from=None
resume_from='work_dirs/stream_petr_vov_flash_800_bs8_seq_24e_mod/latest.pth'
